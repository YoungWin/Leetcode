...
public:
    int change(int amount, vector<int>& coins) {
        int N = coins.size();
        vector<vector<int>> dp(N+1, vector<int>(amount+1, 0));   //用前i个硬币 凑出来金额j，对应的组合数
        for(int i=0; i<=N; i++) dp[i][0]=1;
        for(int i=1; i<=N; i++){
            for(int j=1; j<=amount; j++){
                if(j < coins[i-1])   //第i枚硬币金额 比凑的目标额度j 还要大
                    dp[i][j] = dp[i-1][j];
                else
...
public:
    bool canPartition(vector<int>& nums) {
        ...
        sum=sum/2;
        vector<vector<bool>> dp(N+1, vector<bool>(sum+1,false));   //用前N个数 凑出来总和sum
        for(int i=0; i<=N; i++) dp[i][0]=true;   //用前i个数 凑出来总和0

        for(int i=1; i<=N; i++){
            for(int j=1; j<=sum; j++){
                if(j < nums[i-1]) dp[i][j] = dp[i-1][j];   //第i个数 比总和j 还要大
